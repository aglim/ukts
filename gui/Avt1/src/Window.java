...
class Window {
	...
	/* Add a new mote */
	synchronized void newNode(int nodeId, int val) {
	 ...
	 /* Add a new mote */
	synchronized void updateNode(int nodeId, int val) {
	...
	moteList.setPreferredScrollableViewportSize(new Dimension(200, 400));
    ...
    void newNode(int nodeId, int val) {
	moteListModel.newNode(nodeId, val);
    }

    /* Notification: new data. */
    void newData(int nodeId, int val) {
        moteListModel.updateNode(nodeId, val);
	graph.newData();
    }
